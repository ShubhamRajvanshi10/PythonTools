import os
import optparse
def Generate_InstrProf(Controller_Name, TestRun_Number, Instr_Path):
	...
	with open('Instr_WithOneMS.txt','w+') as IP:
	...
	if (Controller_Name == "None") or (TestRun_Number == "None") or (Instr_Path == "None"):
		...
		if not os.path.exists(Des_Path):
			...
			if os.path.exists(Instr_Path) and os.path.getsize(Instr_Path) > 0:
				#print "AllFine"
				Generate_InstrProf(Controller_Name, TestRun_Number, Instr_Path)
			else:
